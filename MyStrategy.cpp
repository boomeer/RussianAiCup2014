...
constexpr int optSurvive = 4, optOffspring = 4, optStepCount = 4,
		optFinalCount = 4;
...
double stickLength, effStickLength, stickSector, stickSectorTan, holdDist,
		timeGammaSwing;
int maxSwing;
double strikeBase, strikeGrowth, passBase, strikeBeta, passBeta, timeSpread;
Vec2D passSector;
...
void initConsts(const model::Game& game, const model::World& world) {
	...
	rinkWidth = rinkRight - rinkLeft;
	rinkHeight = rinkBottom - rinkTop;
	...
	gridLine = 2 * gridHalfWidth + 1;
	gridHeight = 2 * gridHalfHeight + 1;
	gridCenter = gridHalfHeight * gridLine + gridHalfWidth;
	gridSize = gridHeight * gridLine;

	goalHalf = game.getGoalNetHeight() / 2;
	goalCenter = game.getGoalNetTop() + goalHalf;
	hockeyistRad = world.getHockeyists()[0].getRadius();
	puckRad = world.getPuck().getRadius();
	goalieSpd = game.getGoalieMaxSpeed();
	goalieRange = goalHalf - hockeyistRad;
	...
	accelMin = -game.getHockeyistSpeedDownFactor();
	accelMax = game.getHockeyistSpeedUpFactor();
	turnAngle = game.getHockeyistTurnAngleFactor();

	stickLength = game.getStickLength();
	effStickLength = stickLength / sqrt(3.0);
	stickSector = game.getStickSector() / 2;
	stickSectorTan = tan(stickSector);
	holdDist = game.getPuckBindingRange();
	timeGammaSwing = pow(timeGamma, maxSwing);
	maxSwing = game.getMaxEffectiveSwingTicks();

	strikeBase = game.getStruckPuckInitialSpeedFactor()
			* game.getStrikePowerBaseFactor();
	strikeGrowth = game.getStruckPuckInitialSpeedFactor()
			* game.getStrikePowerGrowthFactor();
	passBase = game.getStruckPuckInitialSpeedFactor()
			* game.getPassPowerFactor();
...
inline int gridPos(const Vec2D &pos) {
	Vec2D offs = (pos - rinkCenter) / cellSize
			+ Vec2D(gridHalfWidth + 0.5, gridHalfHeight + 0.5);
...
struct UnitState {
	...
	bool bounceCircle(const Vec2D &center, double rad, double bounce,
			double frict = 0) {
		Vec2D delta = center - pos;
		if (!(delta.sqr() < rad * rad))
			return false;
		double normSpd = max(0.0, spd * delta), normImp = (1 + bounce)
				* normSpd;
		double tanImp = frict * normImp;
		tanImp = max(-tanImp, min(tanImp, spd % delta)); // TODO: incorrect tanImp

		double invLen2 = 1 / delta.sqr(), invLen = sqrt(invLen2), depth = rad
				* invLen - 1;
		if ((normImp - normSpd) * invLen2 < depth)
			pos -= depthFactor * (depth - minDepth * invLen) * delta;
...
struct StickSector {
	...
	void set(const Vec2D &pos_, const Vec2D &dir_, double back_,
			const Vec2D &forw_, double angle) {
		pos = pos_;
		dir = dir_;
		side = sincos(angle + stickSector);
		back = back_;
		double phi = angle + stickSector / 2, dr = forw_.y / sin(phi);
		forw = forw_.x - dr * cos(phi);
		base = 0.5 * (stickLength + dr);
		coeff = 0.5 / (stickLength + dr);
	}

	double distance(const Vec2D &pt) const {
		double dot = (pt - pos) * dir, cross = (pt - pos) % dir;
		return max((sqr(dot - forw) + sqr(cross)) * coeff - base,
				side.x * abs(cross) - side.y * (dot + back));
	}
};
...
struct HockeyistInfo {
	...
	HockeyistInfo(const model::Hockeyist& hockeyist) :
			id(hockeyist.getId()) {
	}
	...
	void set(const model::Hockeyist& hockeyist, long long puckOwner,
		...
		accelMin = ::accelMin * agi;
		accelMax = ::accelMax * agi;
		turnAngle = ::turnAngle * agi;
		maxTurnSteps = (pi / 2) / turnAngle;
		maxTurnCount = int(maxTurnSteps);
		turnRot = sincos(turnAngle);
		endTurnRot = sincos(turnAngle * (maxTurnSteps - maxTurnCount));

		strikeBase = ::strikeBase * str;
		strikeGrowth = ::strikeGrowth * str;
		passBase = ::passBase * str;
		strikeBeta = ::strikeBeta * dex;
		passBeta = ::passBeta * dex;

		attack[HOLD] = ::strikeChance + max(str, dex);
		attack[FLY] = ::strikeChance + max(dex, agi);
		defence = max(end, agi);
		...
		rot.resize(maxLookahead + 1);
		rot[0] = sincos(angSpd);
		for (int i = 1; i <= maxLookahead; i++) {
			angSpd -= angularFrict * angSpd;
			rot[i] = sincos(angSpd);
		}

		UnitState state;
		state.set(hockeyist);
		Vec2D dir = sincos(hockeyist.getAngle());
		stick.resize(stickLookahead + 1);
		stick[0].set(state.pos, dir, 0, Vec2D(0, 0), 0);
		double back = 0, backSpd = 0;
		Vec2D forw(0, 0), forwSpd(0, 0), forwDir(1, 0);
		for (int i = 1; i <= stickLookahead; i++) {
			state.spd -= hockeyistFrict * state.spd;
			state.pos += state.spd;
			dir = rotate(dir, rot[i]);
			if (i > knockdown) {
				backSpd -= accelMin;
				backSpd -= hockeyistFrict * backSpd;
				back += backSpd;
				forwSpd += accelMax * forwDir;
				forwSpd -= hockeyistFrict * forwSpd;
				forw += forwSpd;
				forwDir = rotate(forwDir, turnRot);
			}
			stick[i].set(state.pos, dir, back, forw,
					(i - knockdown) * turnAngle);
		}
	}
};
...
struct HockeyistState: public UnitState {
	...
	void nextStep(const HockeyistInfo &info, double accel, const Vec2D &turn,
			int time) {
		spd += accel * dir;
		spd -= hockeyistFrict * spd;
		...
		if ((delta = rinkLeft + hockeyistRad - pos.x) > 0) {
			...
			if (offs < 0) {
				if (delta > +1)
					pos.x = rinkLeft - hockeyistRad - 1;
			} else if (!bounceCircle(
					Vec2D(rinkLeft, goalCenter + copysign(goalHalf, sign)),
					hockeyistRad, hockeyistBounce)
					&& !bounceCircle(
							Vec2D(rinkLeft,
									goalCenter + copysign(goalHalf + 25, sign)),
							hockeyistRad, hockeyistBounce / 10)) {
				if (offs > 25) {
					if (spd.x < 0)
						spd.x *= -hockeyistBounce;
					if (spd.x < delta)
						pos.x += depthFactor * (delta + minDepth);
				} else {
					if (spd.x < 0)
						spd.x *= -hockeyistBounce / 10;
					if (spd.x < delta)
						pos.x += depthFactor * (delta + minDepth);
				}
			}
		...
		if ((delta = rinkRight - hockeyistRad - pos.x) < 0) {
			...
			if (offs < 0) {
				if (delta < -1)
					pos.x = rinkRight - hockeyistRad - 1;
			} else if (!bounceCircle(
					Vec2D(rinkRight, goalCenter + copysign(goalHalf, sign)),
					hockeyistRad, hockeyistBounce)
					&& !bounceCircle(
							Vec2D(rinkRight,
									goalCenter + copysign(goalHalf + 25, sign)),
		...
		if ((delta = rinkTop + hockeyistRad - pos.y) > 0) {
			if (spd.y < 0)
				spd.y *= -hockeyistBounce;
			if (spd.y < delta)
				pos.y += depthFactor * (delta - minDepth);
		}
...
struct SimplePuckState: public UnitState {
	...
	bool nextStep() {
		spd -= puckFrict * spd;
		spdLen -= puckFrict * spdLen;
		pos += spd;
		if (abs(pos.x - rinkCenter.x) > rinkWidth / 2 + cellSize)
			return false;
...
struct PuckState: public UnitState {
	...
	int nextStep(int time) {
		double y = max(goalCenter - goalieRange,
				min(goalCenter + goalieRange, pos.y));
		for (int i = 0; i < 2; i++)
			goalie[i].y = max(goalie[i].y - goalieSpd,
					min(goalie[i].y + goalieSpd, y));
		...
		if (time <= goalieTime)
			for (int i = 0; i < 2; i++)
				bounceCircle(goalie[i], hockeyistRad + puckRad, hockeyistBounce,
						goalieFrict);
...
inline Vec2D puckPos(const HockeyistInfo &info, const HockeyistState &state,
		int time) {
	return state.pos + hockeyistFrict * state.spd
			+ holdDist * rotate(state.dir, conj(info.rot[time]));
}
...
template<typename T, typename State, AddPointFunc<T, State> addPoint,
		ClosePathFunc<T, State> closePath> struct Mapper {
	T &obj;
	const HockeyistInfo &info;

	Mapper(T &obj_, const HockeyistInfo &info_) :
			obj(obj_), info(info_) {
	}
	...
	void fillMapTail(const State &state, int flags, double accel, int time) {
		...
		for (int i = time + 1; i <= maxLookahead; i++) {
			cur.nextStep(info, accel, Vec2D(1, 0), i);
			(obj.*addPoint)(info, cur, i, flags, time);
		}
		(obj.*closePath)(info, cur, flags, time);
	}

	void fillMapTail(const State &state, int flags, double accel,
			const Vec2D &turn, const Vec2D &endTurn, int time) {
		...
		for (i++; i <= maxLookahead; i++) {
			if (i > end) {
				cur.nextStep(info, accel, endTurn, i);
				(obj.*addPoint)(info, cur, i, flags, turnTime);
				break;
			}
			cur.nextStep(info, accel, turn, i);
			(obj.*addPoint)(info, cur, i, flags, i);
		}
		for (i++; i <= maxLookahead; i++) {
			cur.nextStep(info, accel, Vec2D(1, 0), i);
			(obj.*addPoint)(info, cur, i, flags, turnTime);
	...
	void fillMap(const State& state, int flags) {
		...
		for (int i = info.knockdown + 1; i <= n; i++) {
			cur.nextStep(info, accel1, turn, i);
			(obj.*addPoint)(info, cur, i, flags, i);
			if (i % turnStep)
				continue;  // TODO: globalTick ?
			fillMapTail(cur, flags, accel1, i);
			fillMapTail(cur, flags, accel2, turn, endTurn, i);
		}
	}

	void fillMap(const State& state) {
		State cur = state;
		(obj.*addPoint)(info, cur, 0, 0, 0);
		for (int i = 1; i <= info.knockdown; i++) {
			cur.nextStep(info, 0, Vec2D(1, 0), i);
			(obj.*addPoint)(info, cur, i, 0, 0);
		}
...
bool checkGoalie(const Vec2D &pos, double rad) {
	double x = min(pos.y - rinkLeft - hockeyistRad,
			rinkRight + hockeyistRad - pos.y);
	double y = max(0.0, abs(pos.y - goalCenter) - goalHalf);
	return x * x + y * y < sqr(hockeyistRad + rad);
}

bool validPuckPos(const Vec2D &pos) {
	if (pos.x < rinkLeft + puckRad || pos.x > rinkRight - puckRad)
		return false;
	if (pos.y < rinkTop + puckRad || pos.y > rinkBottom - puckRad)
		return false;
...
struct EnemyMap {
	...
	void reset() {
		for (int i = 0; i < activeCount; i++) {
			...
			for (auto &flag : stick[i])
				flag = maxLookahead;
		}
	}

	void addMapPoint(const HockeyistInfo &info, HockeyistState &state, int time,
			int flags, double turnTime) {
		auto &cell = stick[info.index][gridPos(
				state.pos + effStickLength * state.dir)];
		cell = min<int>(cell, max(time, info.cooldown));
	}

	void closePath(const HockeyistInfo &info, HockeyistState &state, int flags,
			double turnTime) {
	}

	void fillMap(const HockeyistInfo &info, const HockeyistState &state) {
		Mapper<EnemyMap, HockeyistState, &EnemyMap::addMapPoint,
				&EnemyMap::closePath>(*this, info).fillMap(state);
	}

	static void filterField(vector<int> &field) {
		constexpr int border = 1;

		vector<int> old(gridSize);
		swap(old, field);
	...
	double getDanger(const Vec2D &pos) {
		Vec2D offs = (pos - rinkCenter) / cellSize
				+ Vec2D(gridHalfWidth, gridHalfHeight);
		int x = int(offs.x), y = int(offs.y), cell = y * gridLine + x;
		offs -= Vec2D(x, y);
		double val1 = danger[cell] + (danger[cell + 1] - danger[cell]) * offs.x;
		cell += gridLine;
	...
	void updateSafety(Safety &res, const HockeyistInfo &info,
			const HockeyistState &state, int time, bool havePuck);

	int enemyTime(int cell) const {
		int res = maxLookahead;
		for (int i = 0; i < activeCount; i++)
			res = min(res, stick[i][cell]);
		return res;
	}
};
...
template<bool ally> struct StateScore: public HockeyistState {
	...
	StateScore(const HockeyistState &state) :
			HockeyistState(state), timeFactor(1) {
	}

	void nextStep(const HockeyistInfo &info, double accel, const Vec2D &turn,
			int time) {
		HockeyistState::nextStep(info, accel, turn, time);
		timeFactor *= timeGamma;
	}
...
template<> struct StateScore<true> : public HockeyistState {
	...
	StateScore(const HockeyistState &state) :
			HockeyistState(state), timeFactor(1), safety(1) {
	}
...
struct GoalHelper {
	...
	GoalHelper(const Vec2D &pos, const Vec2D &spd, double power_, bool right) :
		...
		if (right) {
			start.x = 2 * rinkCenter.x - start.x;
			bonus.x = -bonus.x;
		}
		if (pos.y < goalCenter) {
			start.y = 2 * goalCenter - start.y;
			bonus.y = -bonus.y;
		}
	}
	...
	bool init() {
		...
		dist = start.x - hockeyistRad;
		if (!(dist > rad))
			return false;
		offs = max(0.0, start.y - goalHalf - goalieRange);
		dir = start = normalize(start);
		return true;
	}
	...
	template<bool first> bool iterate() {
		...
		if (first) {
			cmp = dist * dir.y - rad;
			if (!(goalieSpd * end < cmp))
				return false;
		}
		double mul = 1 / dir.x, beg = offs * invSpd / dir.y;
		end *= mul;
		if (first) {
			cmp *= mul;
			if (!(goalieSpd * (end - beg) + offs < cmp))
				return false;
		}
		beg = -log(1 - puckBeta * beg) / puckBeta;
		end = -log(1 - puckBeta * end) / puckBeta;
		double len = goalieSpd * (end - beg) + offs;
		if (first && !(len < cmp))
			return false;
...
Sector estimateGoalAngle(const Vec2D &pos, const Vec2D &spd, double power,
		bool right) {
	GoalHelper helper(pos, spd, power, right);
	if (goalieTime > 0) {
		if (!helper.init())
			return Sector();
		if (!helper.iterate<true>())
			return Sector();
		for (int i = 0; i < 3; i++)
			helper.iterate<false>();
	} else
		helper.withoutGoalie();
	return helper.result(pos, right);
}

void interception(Safety &res, const Vec2D &pos, const Vec2D &spd, int time,
		int duration) {
	SimplePuckState puck(pos, spd);
	for (int i = 0; i < duration; i++) {
		enemyMap.updateSafety(res, puck.pos, puck.spdLen, time + i);
		if (!puck.nextStep())
			break;
	}
}
...
struct StrikeInfo {
	...
	double score, passPower;
	Vec2D passDir, strikePos;

	StrikeInfo() :
			strikeTime(maxLookahead), swingTime(0), targetIndex(-1), score(0), passPower(
					1), passDir { 0, 0 } {
	}

	StrikeInfo(int strikeTime_, double score_) :
			strikeTime(strikeTime_), swingTime(0), targetIndex(-1), score(
					score_), passPower(1), passDir { 0, 0 } {
	}
	...
	void tryPickup(double strike, int time, double val, const Safety &safety) {
		...
		val *= safety.multiply(
				currentDanger
						+ passMultiplier
								* (1 + double(deltaTime) / maxLookahead));
		val *= toChance(strike - pickChanceDelta[puckPath[time].status]);
		if (!(val > score))
			return;
	...
	template<bool ally> void evaluateStrike(const HockeyistState &state,
			const Vec2D &puck, double power, double beta, int swing, int time,
			double mul, const Safety &safety, double bonus = 0) {
		double val = 0;
		Vec2D delta(1, 0);
		Sector res = estimateGoalAngle(puck, state.spd, power,
				ally == leftPlayer);
		if (res.span > 0) {
			delta = rotate(res.dir, conj(state.dir));
			Vec2D offs(delta.x, abs(delta.y));
			if (swing < 0)
				offs = (offs.x > passSector.x ?
						Vec2D(1, 0) : rotate(offs, conj(passSector)));
			if (offs.x > 0) {
				val = mul * probFactor(beta, offs.y, res.span);
				if (ally) {
					if (!(safety.multiply(val) + bonus > score))
						return;
					Safety intact = safety;
					interception(intact, puck,
							(power + state.spd * res.dir) * res.dir,
							time + swing, lround(res.time));
					val = intact.multiply(val);
				} else
					val = safety.multiply(val);
			}
		}
		if (!(val + bonus > score))
			return;
	...
	void evaluatePass(const HockeyistState &state, const Vec2D &puck,
			double power, double x, double y, double spd, int catchTime,
			int time, double val, const Safety &safety, int target) {
		Vec2D delta = Vec2D(x, y) - puck;
		double len = delta.len();
		delta /= len;
		Vec2D offs = rotate(delta, conj(state.dir));
		if (!(offs.x > passSector.x))
			return;

		double puckSpd = puckBeta * len
				/ (1 - exp(-puckBeta * (catchTime - time)));
		double relSpd = state.spd * delta;
		if (!(puckSpd < power + relSpd))
			return;
		val *= maxChance
				- max(0.0, puckSpd - spd - puckBeta * len) * chanceDrop;
		if (!(safety.multiply(val) > score))
			return;

		double duration = 1 - puckBeta * len / puckSpd;
		if (!(duration > 0))
			return;
		...
		if (min(x, puck.x) < rinkLeft + (2 * hockeyistRad + puckRad)) {
			double cross = delta
					% (Vec2D(rinkLeft + hockeyistRad, goalCenter) - puck);
			double cmp = abs(delta.x) * goalieRange + (hockeyistRad + puckRad);
			if (cross > -cmp && cross < cmp)
				return;
		}
		if (max(x, puck.x) > rinkRight - (2 * hockeyistRad + puckRad)) {
			double cross = delta
					% (Vec2D(rinkRight - hockeyistRad, goalCenter) - puck);
			double cmp = abs(delta.x) * goalieRange + (hockeyistRad + puckRad);
			if (cross > -cmp && cross < cmp)
				return;
		}
		...
		if (!((val = intact.multiply(val)) > score))
			return;
		...
		targetIndex = target;
		score = val;
		passPower = (puckSpd - relSpd) / power;
		passDir = offs;
	}

	void tryPass(const HockeyistState &state, const Vec2D &puck, double power,
			int time, double mul, const Safety &safety);

	template<bool ally> void tryStrike(const HockeyistInfo &info,
			const StateScore<ally> &state, int time) {
		double mul = state.timeFactor;
		Vec2D puck;
		Safety safety = state.intact();
		if (time + maxSwing <= maxLookahead
				&& safety.multiply(mul * timeGammaSwing) > score)
			do {
				StateScore<ally> cur = state;
				for (int i = 1; i <= maxSwing; i++)
					cur.nextStep(info, 0, Vec2D(1, 0), time + i);

				double val = cur.timeFactor;
				Safety safety = cur.intact();
				if (!puckPathLen)
					puck = puckPos(info, cur, time + maxSwing);
				else {
					if (time >= puckPathLen || !inStrikeSector(cur, puck =
							puckPath[time + maxSwing].pos))
						break;
					double strike =
							info.attack[puckPath[time + maxSwing].status]
									- puckPath[time + maxSwing].chanceDrop;
					safety.update(puckPath[time + maxSwing].intact);
					val *= toChance(strike);
				}
				if (safety.multiply(val) > score)
					evaluateStrike<ally>(cur, puck,
							info.strikeBase + info.strikeGrowth * maxSwing,
							info.strikeBeta, maxSwing, time, val, safety);
			} while (false);

		if (!puckPathLen) {
			puck = puckPos(info, state, time);
			if (ally)
				tryPass(state, puck, info.passBase, time, mul, safety);
		} else {
			if (time >= puckPathLen || !inStrikeSector(state, puck =
					puckPath[time].pos))
				return;
			double strike = info.attack[puckPath[time].status]
					- puckPath[time].chanceDrop;
			safety.update(puckPath[time].intact);
			if (ally)
				tryPickup(strike, time, mul, safety);
			mul *= toChance(strike);
		}
		if (info.havePuck) {
			if (!(safety.multiply(mul) > score))
				return;
			evaluateStrike<ally>(state, puck, info.passBase, info.passBeta, -1,
					time, mul, safety);
		} else {
			double bonus = 0;
			if (ally && puckPathLen) {
				double autoGoal = 0;
				Sector res = estimateGoalAngle(puck, state.spd, info.strikeBase,
						!leftPlayer);
				if (res.span > 0) {
					Vec2D offs = rotate(res.dir, conj(state.dir));
					if (offs.x > 0)
						autoGoal = probFactor(info.strikeBeta, abs(offs.y),
								res.span);
				}
				bonus = safety.multiply(mul * currentDanger) * (1 - autoGoal);
			}
			if (!(safety.multiply(mul) + bonus > score))
				return;
			evaluateStrike<ally>(state, puck, info.strikeBase, info.strikeBeta,
					0, time, mul, safety, bonus);
		}
	}

	template<bool ally> void trySwing(const HockeyistInfo &info,
			const StateScore<ally> &state, int swing, int time) {
		double val = state.timeFactor;
		Vec2D puck;
		Safety safety = state.intact();
		if (!puckPathLen)
			puck = puckPos(info, state, time);
		else {
			if (!inStrikeSector(state, puck = puckPath[time].pos))
				return;
			double strike = info.attack[puckPath[time].status]
					- puckPath[time].chanceDrop;
			safety.update(puckPath[time].intact);
		...
		if (safety.multiply(val) > score)
			evaluateStrike<ally>(state, puck,
					info.strikeBase + info.strikeGrowth * swing,
					info.strikeBeta, time, 0, val, safety);
	}
...
struct MovePlan: public StrikeInfo {
	...
	struct Helper {
		...
		Helper(const HockeyistInfo &info, const MovePlan &plan) :
				pos(0) {
			if (plan.flags & MoveFlag::ROTATE_ONLY)
				accBase = accDelta = 0;
			else {
				accBase = info.accelMin;
				accDelta = info.accelMax;
				if (plan.flags & MoveFlag::BACK)
					swap(accBase, accDelta);
				accDelta -= accBase;
			}
			...
			turnStep[0] = (int) floor(turnTime1);
			turnTime1 -= turnStep[0];
			turnStep[1] = (int) floor(turnTime2);
			turnTime2 -= turnStep[1];
			...
			if (turnStep[0] == turnStep[1]) {
				turnStep[1] = maxLookahead;
				rot[1] = sincos(angle1 + angle2);
				rot[2] = rot[4];
			} else {
				rot[1] = sincos(angle1);
				rot[2] = Vec2D(1, 0);
		...
		double accel(int time) const {
			return accBase
					+ accDelta
							* (stepValue(accTime2 - time)
									- stepValue(accTime1 - time));
		}
	...
	MovePlan() :
			StrikeInfo(0, 0), flags(MoveFlag::STOP), firstTurnEnd(0), secondTurnStart(
					0) {
	}

	MovePlan(int flags_, double turnTime, int strikeTime, double score) :
			StrikeInfo(strikeTime, score), flags(flags_ | MoveFlag::STOP), firstTurnEnd(
					turnTime), secondTurnStart(strikeTime) {
	}

	MovePlan(const StrikeInfo &info, int flags_, double turnTime) :
			StrikeInfo(info), flags(flags_), firstTurnEnd(turnTime), secondTurnStart(
					info.strikeTime) {
	}

	void followPlace(const HockeyistInfo &info, const HockeyistState &state,
			const Vec2D &target) {
		Vec2D delta = rotate(target - state.pos, conj(state.dir));
		double turn = atan2(delta.y, delta.x);
		*static_cast<StrikeInfo *>(this) = StrikeInfo(maxLookahead,
				info.staminaBias);
		flags = (turn > 0 ? MoveFlag::FIRST_LEFT : MoveFlag::FIRST_RIGHT)
				| MoveFlag::STOP;
		firstTurnEnd = abs(turn) / info.turnAngle;
		secondTurnStart = maxLookahead;
	}

	static double placeScore(const HockeyistInfo &info, const Vec2D &pos,
			const TargetPlace &place, double dirFactor) {
		return info.followRotCoeff * dirFactor
				- info.followMoveCoeff * (pos - place.pos).sqr();
	}

	void tryPlace(const HockeyistInfo &info, const Vec2D &pos, const Vec2D &end,
			const Vec2D &dir, const TargetPlace &place, int flags_,
			double turnTime, int endTime, double val) {
		Vec2D view = normalize(place.view - pos);
		val += placeScore(info, end, place, dir * view) - endTime;
		if (!(val > score))
			return;

		*static_cast<StrikeInfo *>(this) = StrikeInfo(endTime, val);
		flags = flags_ | MoveFlag::STOP;
		firstTurnEnd = turnTime;
		secondTurnStart = endTime;
	}

	static double substituteScore(const HockeyistInfo &info, const Vec2D &pos) {
		double dx = max(0.0,
				(leftPlayer ? pos.x - rinkCenter.x : rinkCenter.x - pos.x)
						+ 2 * hockeyistRad);
		double dy = max(0.0, pos.y - rinkTop), val = info.followMoveCoeff
				* (dx * dx + dy * dy);
		return info.staminaCoeff * (substitutes[0].stamina - info.stamina) - val;
	}

	void trySubstitute(const HockeyistInfo &info, const Vec2D &pos, int index,
			int flags_, double turnTime, int endTime) {
		double val = substituteScore(info, pos) - endTime;
		if (!(val > score))
			return;

		*static_cast<StrikeInfo *>(this) = StrikeInfo(endTime, val);
		targetIndex = index;
		flags = flags_ | MoveFlag::STOP | MoveFlag::SUBSTITUTE;
		firstTurnEnd = turnTime;
		secondTurnStart = endTime;
	}

	void updatePlace(const HockeyistInfo &info, const HockeyistState &state,
			const TargetPlace &place, double val) {
		...
		if (flags & MoveFlag::SUBSTITUTE) {
			score = substituteScore(info, end) - strikeTime;
			return;
		}

		Vec2D view = normalize(place.view - cur.pos);
		if ((flags & MoveFlag::ROTATE_ONLY) || !strikeTime) {
			Vec2D delta = rotate(view, conj(state.dir));
			double turn = atan2(delta.y, delta.x);
			flags = (turn > 0 ? MoveFlag::FIRST_LEFT : MoveFlag::FIRST_RIGHT)
					| MoveFlag::ROTATE_ONLY | MoveFlag::STOP;
			firstTurnEnd = abs(turn) / info.turnAngle;
			secondTurnStart = strikeTime = int(firstTurnEnd + 1);
			score = val + placeScore(info, end, place, 1);
		} else
			score = val + placeScore(info, end, place, cur.dir * view)
					- strikeTime;
	}

	template<bool ally> void evaluate(const HockeyistInfo &info,
			const HockeyistState &state) {
	...
	template<bool ally> void evaluateSwing(const HockeyistInfo &info,
			const HockeyistState &state, int swing) {
		reset();
		StateScore<ally> cur = state;
		trySwing<ally>(info, cur, swing, 0);
		for (int i = swing + 1; i <= maxSwing; i++) {
			cur.nextStep(info, 0, Vec2D(1, 0), i);
			trySwing<ally>(info, cur, i, i - swing);
		}
	}
	...
	static int mutate(double &val, double step, double minVal, double maxVal) {
		val += step * frand();
		if (val < minVal) {
			val = min(maxVal, 2 * minVal - val);
			return -1;
		}
		if (val > maxVal) {
			val = max(minVal, 2 * maxVal - val);
			return +1;
		}
		return 0;
	}

	MovePlan(const HockeyistInfo &info, const HockeyistState &state,
			const MovePlan &old, double step, bool ally) :
			flags(old.flags), firstTurnEnd(old.firstTurnEnd), secondTurnStart(
					old.secondTurnStart) {
		strikeTime = old.strikeTime;
		int minTurn2 = max(0, strikeTime - info.maxTurnCount);
		if (mutate(firstTurnEnd, step, 0, 2 * info.maxTurnSteps) < 0)
			flags ^= MoveFlag::FIRST_RIGHT;
		if (mutate(secondTurnStart, 4 * step, minTurn2, strikeTime) > 0)
			flags ^= MoveFlag::SECOND_RIGHT;
		if (ally)
			evaluate<true>(info, state);
		else
			evaluate<false>(info, state);
	}
	...
	void execute(const HockeyistInfo &info, model::Move &move,
		...
		if (swinging) {
			move.setAction(
					strikeTime ?
							model::CANCEL_STRIKE :
							(swingTime ? model::SWING : model::STRIKE));
			move.setSpeedUp(0);
			move.setTurn(0);
			return;
		}
		...
		if (strikeTime) {
			Helper helper(info, *this);
			double accel = helper.accel(0);
			move.setSpeedUp(
					accel > 0 ? accel / info.accelMax : -accel / info.accelMin);
			Vec2D rot = helper.turn(0);
			move.setTurn(atan2(rot.y, rot.x));
			if (flags & MoveFlag::SUBSTITUTE) {
				move.setTeammateIndex(targetIndex);
				move.setAction(model::SUBSTITUTE);
			} else
				move.setAction(model::NONE);
			return;
		}
		move.setSpeedUp(0);
		move.setTurn(0);
		if (flags & MoveFlag::STOP)
			...
			move.setAction(model::SWING);
		else if (!swingTime)
			move.setAction(model::STRIKE);
		else if (puckPathLen)
			move.setAction(model::TAKE_PUCK);
		else {
			move.setAction(model::PASS);
	...
	void print(const char *prefix, long long id) {
		...
		cout << ", times: " << firstTurnEnd << '\'' << secondTurnStart << '\''
				<< strikeTime;
		if (!(flags & MoveFlag::STOP)) {
			if (swingTime >= 0)
				cout << ", swing: " << swingTime;
			else if (puckPathLen)
				cout << ", pickup";
			else
				cout << ", pass: " << passDir.x << '\'' << passDir.y
						<< ", power: " << passPower;
			if (targetIndex >= 0)
				cout << ", target: " << targetIndex;
		}
		cout << endl;
	}
};
...
struct PassTarget: public Vec2D, public MovePlan {
	...
	PassTarget(AllyInfo *rcv, const HockeyistInfo &info, const MovePlan &plan) :
			Vec2D(plan.strikePos), MovePlan(plan), receiver(rcv) {
		spd = (info.attack[FLY] - pickChanceDelta[FLY] - maxChance)
				/ chanceDrop;
		score *= passBonus;
	}

	PassTarget(AllyInfo *rcv, const HockeyistInfo &info, const Vec2D &puck,
			int flags, double turnTime, int strikeTime, double score) :
			Vec2D(puck), MovePlan(flags, turnTime, strikeTime, score), receiver(
					rcv) {
		spd = (info.attack[FLY] - pickChanceDelta[FLY] - maxChance)
				/ chanceDrop;
	}
...
void StrikeInfo::tryPass(const HockeyistState &state, const Vec2D &puck,
		double power, int time, double mul, const Safety &safety) {
	double y1 = rinkTop + puckRad, y2 = rinkBottom - puckRad;
	for (size_t i = 0; i < targets.size(); i++) {
		int hit = targets[i].strikeTime + targets[i].swingTime;
		if (hit <= time)
			continue;
		...
		evaluatePass(state, puck, power, targets[i].x, targets[i].y,
				targets[i].spd, hit, time, val, safety, i);
		evaluatePass(state, puck, power, targets[i].x,
				y1 - (targets[i].y - y1) / wallBounce, targets[i].spd, hit,
				time, val, safety, i);
		evaluatePass(state, puck, power, targets[i].x,
				y2 - (targets[i].y - y2) / wallBounce, targets[i].spd, hit,
				time, val, safety, i);
	}
}

template<bool ally> struct OptimizerState: public StateScore<ally>,
		public StrikeInfo {
	OptimizerState(const HockeyistState &state) :
			StateScore<ally>(state) {
	}
};

template<typename T> size_t chooseBest(vector<T> &array, size_t max) {
	auto rend = array.rend();
	make_heap(array.rbegin(), rend);
	size_t n = 0;
	for (; n < max && rend != array.rbegin(); n++, --rend)
		pop_heap(array.rbegin(), rend);
	array.resize(n);
	return n;
}
...
template<bool ally> struct Optimizer {
	...
	void addPoint(const HockeyistInfo &info, State &state, int time, int flags,
			double turnTime) {
		if (time < info.cooldown)
			return;
		if (time < strikeDelta || !(time % strikeStep))
			state.StrikeInfo::tryStrike<ally>(info, state, time); // TODO: globalTick ?
	}

	void closePath(const HockeyistInfo &info, State &state, int flags,
			double turnTime) {
		if (state.score > 0)
			moves.emplace_back(state, flags, turnTime);
	}

	void optimizeMove(const HockeyistInfo &info, const HockeyistState &state,
			double step, int survive, int offspring) {
	...
	void fillMap(const HockeyistInfo &info, const HockeyistState &state) {
		typedef Optimizer<ally> Self;
		assert(!moves.size());
		Mapper<Self, State, &Self::addPoint, &Self::closePath>(*this, info).fillMap(
				state);
	}

	void findBestMove(const HockeyistInfo &info, const HockeyistState &state,
			MovePlan &plan) {
		plan.evaluate<ally>(info, state);
		moves.push_back(plan);
...
struct EnemyInfo: public HockeyistInfo, public HockeyistState, public Optimizer<
		false> {
	MovePlan plan;
	int swinging;

	EnemyInfo(const model::Hockeyist& hockeyist, long long puckOwner, int index) :
			HockeyistInfo(hockeyist) {
		set(hockeyist, puckOwner, index);
	}

	void set(const model::Hockeyist& hockeyist, long long puckOwner,
			int index) {
		HockeyistInfo::set(hockeyist, puckOwner, index);
		HockeyistState::set(hockeyist);
		reset();
		swinging = hockeyist.getSwingTicks();
	}
	...
	void process() {
		if (!puckPathLen)
			enemyMap.fillMap(*this, *this);
		else if (knockdown)
			plan = MovePlan();
		else if (swinging)
			plan.evaluateSwing<false>(*this, *this, swinging);
		else {
			Mapper<EnemyInfo, State, &EnemyInfo::addPoint, &EnemyInfo::closePath>(
					*this, *this).fillMap(*this);
	...
	const MovePlan &choosePuckMove() {
		if (!swinging) {
			Optimizer<false>::fillMap(*this, *this);
			findBestMove(*this, *this, plan);
		} else
			plan.evaluateSwing<false>(*this, *this, swinging);
		return plan;
	...
	bool execute() {
		if (havePuck)
			puckPath[0].set(*this, *this, 0);
		int time = 1;
		HockeyistState state = *this;
		MovePlan::Helper helper(*this, plan);
		for (; time <= plan.strikeTime; time++) {
			state.nextStep(*this, helper.accel(time - 1), helper.turn(time - 1),
					time);
			if (havePuck)
		...
		double power;
		if (plan.swingTime < 0) {
			dir = rotate(dir, plan.passDir);
			power = passBase * plan.passPower;
		} else
			power = strikeBase + strikeGrowth * plan.swingTime;
		power += state.spd * dir;

		PuckState puck(puckPath[time - 1].pos, power * dir);
		int flag = 0;
		if (!havePuck && !inStrikeSector(state, puck.pos))
			return false;
		for (; time <= maxLookahead; time++) {
			if ((flag = puck.nextStep(time)))
				break;
			puckPath[time].set(puck, time);
		}
		puckPathLen = time;
		goalFlag = leftPlayer ? flag : -flag;
		plan.skipTurn();
		return true;
	}
};

struct AllyInfo: public HockeyistInfo, public HockeyistState, public Optimizer<
		true> {
	enum PlanType {
	...
	AllyInfo(const model::Hockeyist& hockeyist, long long puckOwner, int index) :
			HockeyistInfo(hockeyist) {
		set(hockeyist, puckOwner, index);
	}
	...
	void addPoint(const HockeyistInfo &info, State &state, int time, int flags,
		...
		if (substitutes.size() && substitutes[0].stamina > stamina)
			plan[ATTACK].trySubstitute(info, end, substitutes[0].index, flags,
					turnTime, time);
		else if (!enemyPuck)
			plan[ATTACK].tryPlace(info, state.pos, end, state.dir, attackPlace,
					flags, turnTime, time, staminaBias);
		plan[DEFENCE].tryPlace(info, state.pos, end, state.dir, defencePlace,
				flags, turnTime, time, defenceBias);
	...
	bool tryKnockout() {
		if (!enemyPuck || cooldown || !inStrikeSector(*this, puckPath[0].pos))
			return false;

		double autoGoal = 0;
		Sector res = estimateGoalAngle(puckPath[0].pos, spd, strikeBase,
				!leftPlayer);
		if (res.span > 0) {
			Vec2D offs = rotate(res.dir, conj(dir));
			if (offs.x > 0)
				autoGoal = probFactor(strikeBeta, abs(offs.y), res.span);
		}
		if (autoGoal > 1e-3)
			return false;
	...
	const MovePlan &chooseInterceptMove() {
		if (knockdown)
			return plan[activePlan = ATTACK] = plan[DEFENCE] = MovePlan();
		if (swinging) {
			plan[ATTACK] = plan[DEFENCE] = MovePlan();
			plan[MAIN].evaluateSwing<true>(*this, *this, swinging);
			return plan[activePlan = MAIN];
		}

		activePlan = ATTACK;
		if (enemyPuck)
			plan[ATTACK].followPlace(*this, *this,
					puckPath[0].pos + enemyPuck->spd * (0.5 / hockeyistFrict));
		else
			plan[ATTACK].updatePlace(*this, *this, attackPlace, staminaBias);
		plan[DEFENCE].updatePlace(*this, *this, defencePlace, defenceBias);

		stick.resize(gridSize);
		for (auto &flag : stick)
			flag = {0, -1};
		Mapper<AllyInfo, State, &AllyInfo::addPoint, &AllyInfo::closePath>(
				*this, *this).fillMap(*this);
		findBestMove(*this, *this, plan[MAIN]);
		return plan[MAIN];
	}

	const MovePlan &choosePuckMove() {
		if (!swinging) {
			Optimizer<true>::fillMap(*this, *this);
			findBestMove(*this, *this, plan[MAIN]);
		} else
			plan[MAIN].evaluateSwing<true>(*this, *this, swinging);
		return plan[MAIN];
	...
	void substitute(const SubstituteInfo &sub) {
		...
		double x;
		if (leftPlayer)
			x = min(pos.x, rinkCenter.x - 2 * hockeyistRad);
		else
			x = max(pos.x, rinkCenter.x + 2 * hockeyistRad);
...
template<typename T> void synchronize(vector<T> &list, vector<T *> &ref,
		const model::Hockeyist &hockeyist, long long puckOwner) {
	for (auto iter = list.begin();; ++iter)
		if (iter == list.end()) {
			list.emplace_back(hockeyist, puckOwner, ref.size());
			ref.push_back(&*list.rbegin());
			break;
		} else if (iter->id == hockeyist.getId()) {
			iter->set(hockeyist, puckOwner, ref.size());
			ref.push_back(&*iter);
			break;
		}
}
...
void EnemyMap::init() {
	...
	for (int y = -gridHalfHeight; y <= gridHalfHeight; y++)
		for (int x = -gridHalfWidth; x <= gridHalfWidth; x++, cell++) {
			Sector res = estimateGoalAngle(rinkCenter + Vec2D(x, y) * cellSize,
					Vec2D(0, 0), power, !leftPlayer);
			double border = (rinkHeight / 2 + hockeyistRad - abs(y) * cellSize)
					/ abs(res.dir.y * stickLength);
			danger[cell] = stepValue(border)
					* probFactor(strikeBeta, 0, res.span);
		}
	assert(cell == gridSize);
}

void EnemyMap::updateSafety(Safety &res, const Vec2D &puck, double spd,
		int time) {
	double val = (1 - min(maxChance, 1 + strikeChance - chanceDrop * spd))
			* (1 - getDanger(puck));
	if (time > stickLookahead) {
		int cell = gridPos(puck);
		for (int i = 0; i < activeCount; i++)
			if (stick[i][cell] <= time)
				res.update(i, val);
	} else
		for (auto &enemy : enemies)
			if (time >= enemy->cooldown)
				res.update(enemy->index,
						1 - (1 - val) * stickFactor(enemy->stick[time], puck));
}

void EnemyMap::updateSafety(Safety &res, const HockeyistInfo &info,
		const HockeyistState &state, int time, bool havePuck) {
	...
	if (time > stickLookahead) {
		...
		for (int i = 0; i < activeCount; i++) {
			if (stick[i][cell1] <= time)
				res.update(i, val1);
			if (stick[i][cell2] <= time)
				res.update(i, val2);
		}
	} else
		for (auto &enemy : enemies)
			if (time >= enemy->cooldown) {
				res.update(enemy->index,
						1
								- (1 - val1)
										* stickFactor(enemy->stick[time],
												state.pos));
				res.update(enemy->index,
						1 - (1 - val2) * stickFactor(enemy->stick[time], puck));
			}
}

uint8_t gradient(float val) {
	static const uint8_t palette[26] = { 0, 232, 233, 234, 235, 236, 237, 238,
...
AllyInfo *chooseHockeyist(AllyInfo::PlanType action, double score =
		-numeric_limits<double>::infinity()) {
	AllyInfo *best = nullptr;
	for (auto ally : allies)
		if (!ally->activePlan && ally->plan[action].score > score) {
			score = ally->plan[action].score;
			best = ally;
		}
#ifdef PRINT_LOG
...
void createPlan(PuckState &puck, bool freePuck) {
	...
	attackPlace.pos = rinkCenter
			+ Vec2D(0, avg.y > rinkCenter.y ? -offs : offs);
	attackPlace.view = attackPlace.pos
			+ Vec2D(leftPlayer ? effStickLength : -effStickLength, 0);

	offs = rinkWidth / 2 - (goalieTime > 0 ? goalHalf : hockeyistRad);
	defencePlace.pos = rinkCenter + Vec2D(leftPlayer ? -offs : offs, 0);
	defencePlace.view = defencePlace.pos
			+ effStickLength * normalize(puckPath[0].pos - defencePlace.pos);

	if (goalFlag < 0)
		currentDanger = dangerMultiplier;
	else if (freePuck) {
		EnemyInfo *best = nullptr;
		double score = dangerThreshold;
		for (auto enemy : enemies)
			if (enemy != enemyPuck && enemy->plan.score > score) {
		...
		if (!best || !best->execute()) {
			...
			for (int time = 0; time < puckPathLen - 1; time++) {
				enemyMap.updateSafety(intact, puckPath[time].pos,
						puckPath[time].chanceDrop / chanceDrop, time);
				puckPath[time + 1].intact = intact;
			}
		} else
			currentDanger = dangerMultiplier * best->plan.score;
	} else if (enemyPuck) {
		enemyPuck->choosePuckMove();
		enemyPuck->execute();
	...
	for (auto ally : allies)
		if (ally->havePuck) {
			allyPuck = ally;
			ally->activePlan = AllyInfo::MAIN;
		} else {
			const MovePlan &plan = ally->chooseInterceptMove();
			nextTargets.emplace_back(ally, *ally, plan);
		}
	chooseBest(nextTargets, targetCount);
	swap(targets, nextTargets);
	...
	if (enemyPuck) {
		...
		chooseHockeyist(AllyInfo::MAIN, dangerThreshold);
	} else {
		if (!allyPuck && goalFlag <= 0)
			chooseHockeyist(AllyInfo::MAIN, 0);
		chooseHockeyist(AllyInfo::DEFENCE);
	}

	for (auto ally : allies)
		if (ally->activePlan < AllyInfo::MAIN)
			ally->tryKnockout();
}

void MyStrategy::move(const model::Hockeyist& self, const model::World& world,
		const model::Game& game, model::Move& move) {
	if (globalTick != world.getTick()) {
		int totalGoals = 0;
		for (auto &player : world.getPlayers())
			totalGoals += player.getGoalCount();
		goalieTime = (
				totalGoals ?
						numeric_limits<int>::max() :
						world.getTickCount() - globalTick);

		if (!(globalTick = world.getTick())) {
			initConsts(game, world);
			srand(game.getRandomSeed());
			enemyMap.init();
			substitutes.reserve(MAX_ACTIVE);
			enemyInfo.reserve(6);
			enemies.reserve(MAX_ACTIVE);
			allyInfo.reserve(6);
			allies.reserve(MAX_ACTIVE);
		...
		for (auto &hockeyist : world.getHockeyists()) {
			if (hockeyist.getType() == model::GOALIE)
				puck.goalie[hockeyist.isTeammate() ? 0 : 1] = Vec2D(
						hockeyist.getX(), hockeyist.getY());
			else if (hockeyist.getState() != model::RESTING) {
				if (hockeyist.isTeammate())
					synchronize(allyInfo, allies, hockeyist, owner);
				else
					synchronize(enemyInfo, enemies, hockeyist, owner);
			} else if (hockeyist.isTeammate())
				substitutes.emplace_back(hockeyist);
		}
		...
		if (player.isJustMissedGoal() || player.isJustScoredGoal()) {
			for (size_t i = 0; i < allies.size(); i++)
				if (i < substitutes.size()
						&& allies[i]->stamina < substitutes[i].stamina)
					allies[i]->substitute(substitutes[i]);
				else
					allies[i]->doNothing();
		} else
			createPlan(puck, owner < 0);
	}

	for (auto ally : allies)
		if (ally->id == self.getId()) {
			ally->execute(move);
			return;
		}
}
